        ...
        <form method="post" action="{{url('create')}}" accept-charset="UTF-8">
            ...
            <div class="row mx-2">
                <div class="form-group col-sm-8 ">
                    <input type="url" name="url" id="url" class="form-control" placeholder="Enter your link for shorten" required>
                    @if ($errors->any())
            ...
            <table class="table table-responsive-sm table-striped table-bordered" id="urlShortTable">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{$sno++}}</td>
                        <td>{{$url->url}}</td>
                        <td><a href="{{ route('linkShort',$url->url_id) }}" target="_blank">{{ route('linkShort',$url->url_id) }}</a></td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
